...
| 오후 | 뉴스 수집 서비스 | 네이버 뉴스 검색 (크롤링/RSS) |
| 오후 | 뉴스 수집 테스트 | 코인별 뉴스 검색 테스트 |
...
| 오전 | 프롬프트 설계 | 호재/악재 분류 프롬프트 |
| 오후 | 뉴스 분석 서비스 | NewsAnalysisService 구현 |
| 오후 | 분석 결과 저장 | DB 저장 로직 |

---

#### Day 26: AI 뉴스 분석 (3) - 지표 연동
| 시간 | 작업 | 상세 |
|------|------|------|
| 오전 | 지표 조정 로직 | 분석 결과 → 지표 조정값 계산 |
| 오전 | SignalDetector 연동 | 뉴스 분석 결과 반영 |
| 오후 | 스케줄러 구현 | 3시간마다 뉴스 분석 실행 |
| 오후 | Discord 알림 | 뉴스 분석 결과 DM 발송 |
...
- **분석 대상**: 투자 코인 관련 뉴스
- **적용 방식**: 호재/악재 분석 → 지표 ±2% 조정

#### 처리 흐름
```
1. 뉴스 수집 (투자 코인별)
   - 네이버 뉴스 검색 / 크롤링
   - 코인 관련 키워드 (BTC, 비트코인, ETH 등)
       ↓
2. AI 분석 (Google Gemini API - 무료)
   ...
   - 영향도 점수 (-100 ~ +100)
       ↓
3. 지표 조정 반영
   - 호재: 매수 조건 완화 (buyThresholdPct ↑)
   - 악재: 매수 조건 강화 (buyThresholdPct ↓)
   - 조정 범위: 기준값 대비 ±2%
       ↓
4. 결과 저장 & 알림
   - coin_news_analysis 테이블 저장
   - Discord DM으로 분석 결과 알림
```

#### 지표 조정 기준
| 뉴스 분석 결과 | 영향도 점수 | 지표 조정 | 예시 |
|--------------|-----------|----------|------|
| 강한 호재 | +70 ~ +100 | buyThresholdPct +2% | -5% → -3% |
| 호재 | +30 ~ +69 | buyThresholdPct +1% | -5% → -4% |
| 중립 | -29 ~ +29 | 변동 없음 | -5% 유지 |
| 악재 | -30 ~ -69 | buyThresholdPct -1% | -5% → -6% |
| 강한 악재 | -70 ~ -100 | buyThresholdPct -2% | -5% → -7% |
