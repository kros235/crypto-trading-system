...
#### Day 32: 도움말 시스템 + 초보자 온보딩 가이드
| 시간 | 작업 | 상세 | 상태 |
|------|------|------|------|
| 오전 | 카드별 도움말 아이콘 | 모든 페이지 카드에 ? 아이콘 + 툴팁/다이얼로그 | |
| 오전 | ├ 대시보드 | 10개 카드 도움말 추가 | |
| 오전 | ├ 거래 설정 | 각 설정 항목별 상세 설명 | |
| 오전 | ├ 백테스팅 | 파라미터별 의미 설명 | |
| 오전 | ├ 봇 모니터링 | 기술적 지표 용어 설명 | |
| 오전 | ├ 보유 자산 | 수익 분석 용어 설명 | |
| 오전 | └ 기타 페이지 | 프로필, 보안, 관리자 등 | |
| 오후 | 시작 가이드 위젯 | 대시보드 상단 온보딩 스텝 표시 | |
| 오후 | ├ Step 1 | API 키 등록 안내 (업비트 API 발급 링크 포함) | |
| 오후 | ├ Step 2 | 거래 설정 안내 | |
| 오후 | ├ Step 3 | 소액 테스트 안내 | |
| 오후 | └ 완료 상태 | 각 단계 완료 여부 체크 | |
| 오후 | 도움말/FAQ 페이지 | 자주 묻는 질문 + 용어 사전 | |
| 오후 | ├ 용어 사전 | MA, RSI, 볼린저밴드 등 설명 | |
| 오후 | ├ FAQ 섹션 | 자주 묻는 질문 10~15개 | |
| 오후 | └ 문제 해결 가이드 | 일반적인 오류 해결 방법 | |
...
### 도움말 시스템 상세 (Day 32 신규)

#### 개요
모든 페이지의 카드에 도움말 아이콘(?)을 추가하여 초보 사용자가 각 기능을 쉽게 이해할 수 있도록 지원

#### 구현 방식
| 방식 | 설명 | 적용 대상 |
|------|------|----------|
| **툴팁** | 마우스 호버 시 짧은 설명 표시 | 간단한 용어/항목 |
| **다이얼로그** | 클릭 시 상세 설명 팝업 | 복잡한 기능/설정 |

#### 페이지별 도움말 항목

**1. 대시보드 (DashboardView)**
| 카드 | 도움말 내용 |
|------|------------|
| 사용자 정보 | 계정 상태, 투자 기간 계산 방법 |
| 통계 카드 (4개) | 총 손익, 평가액, 매수/매도 건수 의미 |
| 봇 상태 | 자동매매 동작 방식, 실행 주기 |
| 업비트 잔고 | 실제 업비트 계좌 연동 상태 |
| 거래 설정 요약 | 현재 적용된 설정 요약 |
| 일일 한도 | 한도 계산 방식, 진행률 의미 |
| 매수 조건 | 이격도, 목표가 계산 방법 |
| 수익 현황 | 평가/실현/누적 수익 차이 |
| 자산 변동 차트 | 차트 해석 방법 |
| 시스템 알림 | 알림 종류별 의미 |

**2. 거래 설정 (TradingSettingsView)**
| 섹션 | 도움말 내용 |
|------|------------|
| 거래 코인 선택 | 추천 코인 수, 분산 투자 안내 |
| 기술적 지표 | MA, RSI, 볼린저밴드 설명 |
| 매수 조건 | 하락률, 최대 보유 건수 의미 |
| 매도 조건 | 목표 수익률, 손절매, 트레일링 스톱 |
| 리스크 관리 | 일일 한도, 종목 비중, 긴급 정지 |
| 급락장 보호 | 시장 필터, 누적 손실, 연속 손절 |
| AI 뉴스 분석 | 가중치 적용 방식 |
...
| 고급 설정 | 각 파라미터 최적값 안내 |
| 리스크 관리 | 백테스트용 설정 의미 |
| 결과 해석 | 수익률, MDD, 샤프비율 의미 |

**4. 봇 모니터링 (BotMonitorView)**
| 섹션 | 도움말 내용 |
|------|------------|
| 봇 상태 | 실행 중/대기 중 상태 의미 |
| 기술적 지표 | RSI, BB, MA 값 해석 방법 |
| 매수/매도 신호 | 신호 발생 조건 |
| 수동 제어 | 수동 실행 시 주의사항 |

**5. 보유 자산 (HoldingsView)**
| 섹션 | 도움말 내용 |
|------|------------|
| 기간별 수익 | 각 기간 계산 기준 |
| 코인별 수익 | 승률, 평균 수익 의미 |
...
| 페이지 | 도움말 항목 |
|--------|------------|
| 프로필 설정 | API 키 등록 방법, IP 화이트리스트 |
| 계정 보안 | 2FA 설정 방법, 보안 권장사항 |
| 일일 리포트 | 리포트 항목 의미 |
| 코인 뉴스 | AI 분석 점수 해석 |
| 관리자 | 시스템 메트릭 의미 |
...
│   └ ⭐ 발급 링크: https://upbit.com/mypage/open_api_management │
│                                                         │
│ Step 2: 거래 설정  ────────────────────── [진행 중]     │
│   └ 거래할 코인과 매수/매도 조건을 설정하세요             │
│                                                         │
│ Step 3: 소액 테스트  ──────────────────── [대기]        │
│   └ 10만원 이하로 1~2일간 테스트 후 본격 운영            │
└─────────────────────────────────────────────────────────┘
```
...
- 첫 로그인 시 자동 표시
- 각 단계 완료 시 체크 표시
- "다시 보지 않기" 선택 시 localStorage 저장
- 대시보드 설정에서 다시 활성화 가능

#### 도움말/FAQ 페이지 (HelpView)
...
| 파일 | 변경 내용 |
|------|----------|
| `components/HelpIcon.vue` | 재사용 가능한 도움말 아이콘 컴포넌트 (신규) |
| `components/HelpDialog.vue` | 도움말 다이얼로그 컴포넌트 (신규) |
| `components/StartGuide.vue` | 시작 가이드 위젯 컴포넌트 (신규) |
| `views/HelpView.vue` | 도움말/FAQ 페이지 (신규) |
| `views/DashboardView.vue` | 시작 가이드 위젯 추가 |
| `views/*.vue` | 각 페이지 카드에 HelpIcon 추가 |
| `router/index.ts` | /help 라우트 추가 |
| `components/TheSidebar.vue` | 도움말 메뉴 추가 |
| `data/helpContent.ts` | 도움말 텍스트 데이터 (신규) |
...
<v-card-title>
  봇 상태
  <HelpIcon 
    tooltip="자동매매 봇의 현재 상태입니다. 5분마다 자동 실행됩니다."
    :dialog="true"
    dialog-title="봇 상태 안내"
    :dialog-content="botStatusHelpContent"
  />
</v-card-title>
```
---

### 라운드로빈 매수 방식 상세 (Backlog - 미구현)

#### 개요
현재 순차 처리 방식을 **라운드로빈 방식**으로 개선하여 분산 투자 효과 극대화

#### 현재 방식 vs 라운드로빈 비교

| 항목 | 현재 (순차) | 라운드로빈 |
|------|------------|-----------|
| 분배 방식 | 코드 순서대로 선착순 | 균등 분배 |
| 한도 부족 시 | 앞 코인만 매수 | 모든 코인에 분산 |
| 우선순위 기준 | 코드 순서 | **신호 강도** |
| 리스크 분산 | 불리 (집중 투자) | 유리 (분산 투자) |
| 한도 활용률 | 비중 제한 시 미사용 잔여 | **100% 재분배** |
...
**시나리오: 남은 한도 10만원, ETH/XRP 매수 신호 발생**
| 방식 | ETH | XRP | 총 매수 | 특징 |
|------|-----|-----|--------|------|
| 현재 (순차) | 10만원 | 0원 | 10만원 | ETH 집중 |
| **라운드로빈** | **5만원** | **5만원** | 10만원 | **분산 투자** |

**시나리오: 남은 한도 1만원, BTC(MEDIUM)/ETH(STRONG)/SOL(WEAK) 매수 신호 발생**
| 방식 | BTC | ETH | SOL | 총 매수 | 선정 기준 |
|------|-----|-----|-----|--------|----------|
| 현재 (순차) | 1만원 | 0원 | 0원 | 1만원 | 코드 순서 |
| **라운드로빈** | **5,000원** | **5,000원** | 0원 | 1만원 | **신호 강도** |

→ 균등 분배 시 3,333원 < 최소 5,000원이므로, 신호 강도 순 상위 2개(ETH, BTC)만 선정

#### 신호 강도 정의
| 강도 | 값 | 조건 |
|------|-----|------|
| **STRONG** | 3 | 4개 조건 모두 충족 (MA + RSI + BB + 거래량) |
| **MEDIUM** | 2 | 3개 조건 충족 |
| **WEAK** | 1 | 2개 조건 충족 |

#### 수정 대상 파일
- `backend/src/main/java/com/cryptotrading/service/TradingBotService.java`
- `backend/src/main/java/com/cryptotrading/service/RiskManagementService.java`
- `backend/src/main/java/com/cryptotrading/dto/bot/TradingSignalDTO.java` (신호 강도 필드 추가)
...
| 라운드로빈 매수 방식 | 매수 신호 코인들에 균등 분배 투자 | 🟡 권장 | 미구현 |
